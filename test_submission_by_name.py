...
from util import update_request, fetch_submissions, fetch_submission, load_submissions, filter_status, upload_results, parse_huggingface_url, data_to_saveable_name
from const import *

def main():
  parser = argparse.ArgumentParser()
  parser.add_argument("--hf-repo", type=str, help="URL of huggingface repo.")
  ...
  sub = fetch_submission(args.hf_repo)
  # fetch_submissions()
  ...
  with tempfile.TemporaryDirectory() as temp_dir:
    ...
    method_name_saveable = f"{method_name}_{submit_time}"
    logging.basicConfig(
    ...
    snapshot_download(repo_id=submission_repo, allow_patterns=[f"{submission_prefix}*{suffix}" for suffix in suffixes],
                      revision="main", local_dir=local_temp_dir)
